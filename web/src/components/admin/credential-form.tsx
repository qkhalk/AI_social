  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-lg">
      ...
      <div>
        ...
        <ProviderKeyHint providerName={selectedProvider?.name} />
      </div>
...
const KEY_HINTS: Record<string, { prefix: string; note: string }> = {
  openai:      { prefix: "sk-",      note: "Lay tu platform.openai.com -> API keys" },
  anthropic:   { prefix: "sk-ant-",  note: "Lay tu console.anthropic.com -> Settings -> API Keys" },
  google:      { prefix: "AIza",     note: "Lay tu aistudio.google.com -> API keys (Gemini)" },
  openrouter:  { prefix: "sk-or-",   note: "Lay tu openrouter.ai -> Keys" },
  custom:      { prefix: "(tuy chinh)", note: "Key tuy theo nha cung cap, vi du sk-live-..." },
};

function ProviderKeyHint({ providerName }: { providerName?: string }) {
  if (!providerName || !KEY_HINTS[providerName]) return null;
  const hint = KEY_HINTS[providerName];
  return (
    <p className="text-xs text-blue-400 mt-1">
      Format: bat dau bang <code className="bg-gray-800 px-1 rounded">{hint.prefix}</code> - {hint.note}
    </p>
  );
}
